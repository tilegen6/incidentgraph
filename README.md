...
The separate `test:postgres` check starts a native PostgreSQL instance bound to loopback on port 55432, applies migrations, seeds, and verifies persistence after reconnect. Its data stays in `.data/postgres-verification`. The process is stopped in a `finally` block.

The `docker-smoke` CI job builds the complete Compose stack on Ubuntu, checks frontend assets and the API proxy, signs in, persists an incident and annotation in PostgreSQL, and verifies them after restarting the API container. It removes the disposable stack and volumes afterward. Docker is unavailable on the development Windows host, so container verification runs in GitHub Actions.
